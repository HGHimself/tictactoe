...
fn terminal(board: &Board, length: u32) -> bool {
    let h = heuristic(board, length, &Symbol::X);
    if h > 99999 || h < -99999 {
        true
    } else {
        false
...
fn minimax(node: &Board, length: u32, depth: u32, maximizing: bool, agent: &Symbol) -> i32 {
    if depth == 0 || terminal(node, length) {
        return heuristic(node, length, agent);
    } else {
        if maximizing {
            let max_value = get_all_potential_moves(node, agent).iter().fold((-99999, vec![]), |acc, child| {
            ...
            let min_value = get_all_potential_moves(node, agent).iter().fold((99999, vec![]), |acc, child| {
